...
def get_services():
    ...
    include_transactions = (
        request.args.get("include_transactions", "").lower() == "true"
    )

    with get_db_session() as session:
        services = session.query(TradingService).all()
        result = [
            service.to_dict(include_relationships=include_transactions)
...
def toggle_service(service_id):
    ...
    try:
        ...
        return jsonify({"service_id": service_id, "service_state": new_state})
    except Exception as e:
...
def delete_service(service_id):
    ...
    try:
        ...
        return jsonify({"deleted": True, "service_id": service_id})
    except Exception as e:
