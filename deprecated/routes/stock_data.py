...
def index():
    ...
    return render_template(
        "stock_data.html", supported_stocks=SUPPORTED_SYMBOLS, now=datetime.now()
    )
...
def fetch_market_data(symbol: str):
    ...
    try:
        ...
        if data_type == "intraday":
            ...
            return (
                jsonify({"error": 'Invalid data type. Must be "intraday" or "daily".'}),
                400,
            )
    ...
    try:
        if data_type == "intraday":
            ...
            records = (
                ...
                .join(Stock)
                .order_by(StockIntradayPrice.id.desc())
                .limit(10)
                .all()
            )
            ...
            records = (
                ...
                .join(Stock)
                .order_by(StockDailyPrice.id.desc())
                .limit(10)
                .all()
            )
