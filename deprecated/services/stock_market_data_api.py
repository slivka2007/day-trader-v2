...
def get_intraday_data(
    symbol: str, interval: str = "1m", period: str = "1d"
) -> List[Dict[str, Any]]:
    """
    ...
    try:
        ...
        for data_point in intraday_data:
            ...
            existing = (
                ...
                .filter_by(stock_id=stock.id, timestamp=data_point["timestamp"])
                .first()
            )
            ...
            result.append(
                {
                    ...
                    ),  # Yahoo Finance returns adjusted close as 'Close'
                    "volume": (
                        int(row["Volume"]) if not pd.isna(row["Volume"]) else None
                    ),
                }
    ...
    try:
        ...
        for data_point in daily_data:
            ...
            existing = (
                ...
                .filter_by(stock_id=stock.id, date=data_point["date"])
                .first()
            )
