...
def should_sell(stock_symbol: str, purchase_price: Decimal) -> str:
    ...
    """
    logger.debug(
        f"Running sell decision algorithm for {stock_symbol} purchased at {purchase_price}"
    )
    ...
    if price_diff_percent > 0:
        ...
        sell_probability = min(
            0.9, base_probability + (float(price_diff_percent) / 100)
        )
    ...
    sell_probability = float(sell_probability) + float(
        stock_adjustments.get(stock_symbol, Decimal("0"))
    )
    ...
    if random.random() < sell_probability:
        logger.info(
            f"Sell algorithm recommends selling {stock_symbol} (bought: ${purchase_price}, current: ${current_price}, diff: {price_diff_percent:.2f}%)"
        )
        return DECISION_YES
    else:
        logger.info(
            f"Sell algorithm recommends holding {stock_symbol} (bought: ${purchase_price}, current: ${current_price}, diff: {price_diff_percent:.2f}%)"
        )
        return DECISION_NO
