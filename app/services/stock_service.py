...
class StockService:
    ...
    def update_stock(session: Session, stock: Stock, data: Dict[str, Any]) -> Stock:
        ...
        try:
            ...
            allowed_fields = {"name", "is_active", "sector", "description"}
    ...
    def update_stock_attributes(
        stock: Stock, data: Dict[str, Any], allowed_fields: Optional[Set[str]] = None
    ) -> bool:
        """
    ...
    def change_active_status(session: Session, stock: Stock, is_active: bool) -> Stock:
        ...
        try:
            ...
            if bool(stock.is_active.scalar() != is_active):
    ...
    def toggle_active(session: Session, stock: Stock) -> Stock:
        ...
        return StockService.change_active_status(
            session, stock, not bool(stock.is_active.scalar())
        )
    ...
    def get_latest_price(session: Session, stock: Stock) -> Optional[float]:
        ...
        return (
            float(str(latest_price.close_price))
    ...
    def search_stocks(session: Session, query: str, limit: int = 10) -> List[Stock]:
        ...
        return (
            session.query(Stock)
            .filter(or_(Stock.symbol.ilike(search_term), Stock.name.ilike(search_term)))
            .limit(limit)
            .all()
        )
