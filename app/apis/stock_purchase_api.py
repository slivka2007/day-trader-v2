...
def purchase_stock(stock_symbol: str, available_funds: Decimal) -> Tuple[int, Decimal, datetime]:
    """
    ...
    """
    logger.info(f"Purchasing stock {stock_symbol} with available funds: ${available_funds}")
    ...
    try:
        ...
        current_price = current_price.quantize(Decimal('0.01'))  # Round to 2 decimal places
        ...
        if max_shares <= 0:
            logger.warning(f"Insufficient funds (${available_funds}) to purchase {stock_symbol} at ${current_price}")
            raise StockPurchaseError(f"Insufficient funds (${available_funds}) to purchase {stock_symbol} at ${current_price}")
        
        # Record purchase time
        purchase_time = datetime.utcnow()
        
        logger.info(f"Purchased {max_shares} shares of {stock_symbol} at ${current_price} per share")
