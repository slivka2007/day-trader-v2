...
from app.algorithms.stock_sell_logic import should_sell
...
def should_sell_stock(stock_symbol: str, purchase_price: Decimal) -> Literal['YES', 'NO']:
    ...
    """
    logger.info(f"API: Evaluating whether to sell {stock_symbol} bought at ${purchase_price}")
    ...
    try:
        ...
        logger.error(f"API: Error in sell decision algorithm for {stock_symbol}: {str(e)}")
