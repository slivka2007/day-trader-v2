...
def sell_stock(stock_symbol: str, number_of_shares: int) -> Tuple[Decimal, datetime]:
    ...
    if number_of_shares <= 0:
        raise ValueError(
            f"Cannot sell {number_of_shares} shares, must be a positive number"
        )
    ...
    try:
        ...
        if stock_symbol in MOCK_PRICES:
            ...
            min_change, max_change = PRICE_MOVEMENT.get(
                stock_symbol, (Decimal("-0.1"), Decimal("0.1"))
            )
            ...
            logger.info(
                f"MOCK API: Selling {number_of_shares} shares of {stock_symbol} at {sale_price}"
            )
        ...
        logger.info(
            f"MOCK API: Selling {number_of_shares} shares of {stock_symbol} at {mock_price}"
        )
