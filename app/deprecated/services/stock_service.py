...
from app.deprecated.config.constants import (
    DECISION_YES,
...
from app.deprecated.exceptions.exceptions import InvalidStateError, ServiceError
from app.deprecated.services.database import get_session
from app.deprecated.services.session_manager import SessionManager
from app.deprecated.services.stock_buy_api import should_buy_stock
from app.deprecated.services.stock_purchase_api import purchase_stock
from app.deprecated.services.stock_sale_api import sell_stock
from app.deprecated.services.stock_sell_api import should_sell_stock
...
class StockTradingService:
    ...
    def __init__(
        ...
        try:
            if service_id:
                ...
                self.service = (
                    self.session.query(TradingService)
    ...
    def start_trading_cycle(
        self, polling_interval: int = DEFAULT_POLLING_INTERVAL
    ) -> None:
        """
        ...
        """
        logger.info(
            f"Starting trading cycle for {self.stock_symbol} with service ID {self.service.service_id}"
        )
    ...
    def _execute_buy_cycle(self) -> None:
        ...
        if buy_decision == DECISION_YES:
            ...
            try:
                number_of_shares, purchase_price, date_time_of_purchase = (
                    purchase_stock(self.stock_symbol, self.service.fund_balance)
                )
                ...
                logger.info(
                    f"Purchased {number_of_shares} shares of {self.stock_symbol} at {purchase_price} per share"
                )
    ...
    def _execute_sell_cycle(self) -> None:
        ...
        last_transaction = (
            self.session.query(TradingTransaction)
            .filter_by(service_id=self.service.service_id, sale_price=None)
        ...
        sell_decision = should_sell_stock(
            self.stock_symbol, last_transaction.purchase_price
        )
        ...
        if sell_decision == DECISION_YES:
            ...
            try:
                sale_price, date_time_of_sale = sell_stock(
                    self.stock_symbol, self.service.current_number_of_shares
                )
                ...
                # Calculate gain/loss
                gain_loss = (sale_price - last_transaction.purchase_price) * Decimal(
                    last_transaction.number_of_shares
                )
                last_transaction.gain_loss = gain_loss

                # Update service record
                total_proceeds = (
                    Decimal(self.service.current_number_of_shares) * sale_price
                )
                ...
                logger.info(
                    f"Sold {last_transaction.number_of_shares} shares of {self.stock_symbol} at {sale_price} per share"
                )
    ...
    def restart_trading_cycle(cls, service_id, polling_interval=DEMO_POLLING_INTERVAL):
        ...
        with SessionManager() as session:
            service_model = (
                session.query(TradingService).filter_by(service_id=service_id).first()
            )
