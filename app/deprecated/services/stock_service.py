...
from app.deprecated.exceptions.exceptions import ServiceError, InvalidStateError
...
class StockTradingService:
    ...
    def __init__(self, service_id: Optional[int] = None, stock_symbol: str = "", 
        ...
        try:
            if service_id:
                ...
                self.service = self.session.query(TradingService).filter_by(service_id=service_id).first()
                if not self.service:
    ...
    def restart_trading_cycle(cls, service_id, polling_interval=DEMO_POLLING_INTERVAL):
        ...
        with SessionManager() as session:
            service_model = session.query(TradingService).filter_by(service_id=service_id).first()
