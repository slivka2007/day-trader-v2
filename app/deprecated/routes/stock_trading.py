...
def create_service():
    ...
    try:
        service = StockTradingService(
            stock_symbol=stock_symbol, starting_balance=starting_balance
        )
        ...
        logger.info(
            f"Started trading service for {stock_symbol} with balance {starting_balance}"
        )
...
def stop_service(service_id):
    ...
    try:
        ...
        response = jsonify(
            {"success": True, "message": f"Service {service_id} stopped successfully"}
        )
        response.headers["Content-Type"] = "application/json"
        return response
...
def start_service(service_id):
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Error starting thread for service {service_id}: {str(thread_error)}"
            )
        ...
        response = jsonify(
            {"success": True, "message": f"Service {service_id} started successfully"}
        )
        response.headers["Content-Type"] = "application/json"
        return response
            ...
            result.append(
                {
                    ...
                    "date_time_of_purchase": (
                        t.date_time_of_purchase.isoformat()
                        if t.date_time_of_purchase
                        else None
                    ),
                    "date_time_of_sale": (
                        t.date_time_of_sale.isoformat() if t.date_time_of_sale else None
                    ),
                }
            )
            ...
            result.append(
                {
                    ...
                    "last_start_date": (
                        s.last_start_date.isoformat() if s.last_start_date else None
                    ),
                    "last_stop_date": (
                        s.last_stop_date.isoformat() if s.last_stop_date else None
                    ),
