...
stock_search_model = api.model(
    "StockSearchResults",
    {
        "results": fields.List(
            fields.Nested(stock_model), description="List of matching stocks"
        ),
        ...
        with SessionManager() as session:
            try:
                ...
                stock = StockService.create_stock(
                    session=session, data=cast(Dict[str, Any], validated_data)
                )
        ...
        with SessionManager() as session:
            ...
            try:
                ...
                result = StockService.update_stock(
                    session, stock, cast(Dict[str, Any], validated_data)
                )
        ...
        with SessionManager() as session:
            ...
            try:
                ...
                current_app.logger.warning(
                    f"Business logic error deleting stock: {str(e)}"
                )
                raise
            except Exception as e:
    ...
    def post(self, id):
        ...
        with SessionManager() as session:
            ...
            try:
                ...
                current_app.logger.warning(
                    f"Validation error toggling stock status: {str(e)}"
                )
                raise
            except Exception as e:
                current_app.logger.error(
                    f"Error toggling stock active status: {str(e)}"
                )
