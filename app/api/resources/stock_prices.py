...
daily_price_input_model = api.model(
...
intraday_price_model = api.model(
    "StockIntradayPrice",
    {
        "id": fields.Integer(readonly=True, description="Price record identifier"),
        "stock_id": fields.Integer(description="Stock identifier"),
...
intraday_price_input_model = api.model(
...
daily_price_list_model = api.model(
    "DailyPriceList",
    {
        "items": fields.List(
            fields.Nested(daily_price_model), description="List of daily prices"
        ),
...
intraday_price_list_model = api.model(
    "IntradayPriceList",
    {
        "items": fields.List(
            fields.Nested(intraday_price_model), description="List of intraday prices"
        ),
        "pagination": fields.Nested(
            pagination_model, description="Pagination information"
        ),
...
class StockDailyPrices(Resource):
    ...
    def post(self, stock_id):
        ...
        try:
            ...
            validated_data = cast(
                Dict[str, Any], daily_price_input_schema.load(data_dict)
            )
            ...
            with SessionManager() as session:
                ...
                if validated_data and "price_date" in validated_data:
                    price_date_value = validated_data.get("price_date")
                    if isinstance(price_date_value, str):
                        try:
                            price_date = datetime.strptime(
                                price_date_value, "%Y-%m-%d"
                            ).date()
                        except ValueError:
                            raise ValidationError(
                                "Invalid price_date format. Use YYYY-MM-DD"
                            )
                    else:
                        price_date = price_date_value

                if not price_date:
                    raise ValidationError("Missing or invalid price_date")
            ...
            current_app.logger.error(
                f"Validation error creating price record: {str(e)}"
            )
            raise
...
class StockIntradayPrices(Resource):
            ...
            if "start_time" in request.args:
                try:
                    start_time = datetime.strptime(
                        request.args["start_time"], "%Y-%m-%d %H:%M:%S"
                    )
            ...
            if "end_time" in request.args:
                try:
                    end_time = datetime.strptime(
                        request.args["end_time"], "%Y-%m-%d %H:%M:%S"
                    )
            ...
            if "interval" in request.args:
                try:
                    ...
                    if interval not in [1, 5, 15, 30, 60]:
                        raise ValidationError(
                            "Invalid interval. Use 1, 5, 15, 30, or 60"
                        )
    ...
    def post(self, stock_id):
        ...
        try:
            ...
            data_dict = data if isinstance(data, dict) else {}
            validated_data = cast(
                Dict[str, Any], intraday_price_input_schema.load(data_dict)
            )
            ...
            with SessionManager() as session:
                ...
                if not timestamp:
                    raise ValidationError("Missing or invalid timestamp")

                # Get the interval
                interval = validated_data.get("interval") if validated_data else None
                if not interval:
                    raise ValidationError("Missing interval")
    ...
    def put(self, price_id):
        ...
        try:
            with SessionManager() as session:
                ...
                result = PriceService.update_daily_price(
                    session, price_id, cast(Dict[str, Any], data_dict)
                )
                return daily_price_schema.dump(result)

        except ValidationError as e:
            current_app.logger.error(
                f"Validation error updating price record: {str(e)}"
            )
            raise
        except BusinessLogicError as e:
            current_app.logger.error(f"Business logic error: {str(e)}")
...
class IntradayPriceItem(Resource):
    ...
    def put(self, price_id):
        ...
        try:
            with SessionManager() as session:
                # Update the price record using the service
                result = PriceService.update_intraday_price(
                    session, price_id, cast(Dict[str, Any], data_dict)
                )
