...
class StockDailyPriceDeleteSchema(Schema):
    ...
    def validate_deletion(self, data, **kwargs):
        ...
        with SessionManager() as session:
            price = (
                session.query(StockDailyPrice).filter_by(id=data["price_id"]).first()
            )
            if not price:
            ...
            if price_date is not None and price_date >= thirty_days_ago:
                raise ValidationError(
                    "Cannot delete recent price data (less than 30 days old). This data may be in use for active analyses."
                )
...
class StockIntradayPriceDeleteSchema(Schema):
    ...
    def validate_deletion(self, data, **kwargs):
        ...
        with SessionManager() as session:
            price = (
                session.query(StockIntradayPrice).filter_by(id=data["price_id"]).first()
            )
            if not price:
            ...
            if timestamp is not None and timestamp.date() >= seven_days_ago:
                raise ValidationError(
                    "Cannot delete recent price data (less than 7 days old). This data may be in use for active analyses."
                )
